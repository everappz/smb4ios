#import "SMB4iOSNetBios.h"
#import "SMB4iOSUtils.h"
#import "SMB4iOSAsyncUdpSocket.h"
#import "SMB4iOSNetBiosQuery.h"
...
@interface SMB4iOSNetBiosOperation : NSObject
...
@interface SMB4iOSNetBios () <SMB4iOSAsyncUdpSocketDelegate>
...
@implementation SMB4iOSNetBios{
    SMB4iOSAsyncUdpSocket *_udpSocket;
    NSMutableDictionary<NSNumber *,SMB4iOSNetBiosOperation *> *_operations;
    long _nextOperationTag;
}

+ (SMB4iOSNetBios *) instance{
    static dispatch_once_t onceToken;
    static SMB4iOSNetBios *netBios = nil;
    dispatch_once(&onceToken, ^{
        netBios = [[SMB4iOSNetBios alloc] init];
    });
    return netBios;
}
...
- (instancetype)init{
    self = [super init];
    if (self) {
        _operations = [[NSMutableDictionary<NSNumber *,SMB4iOSNetBiosOperation *> alloc] init];
        _nextOperationTag = 1;
...
- (void) resolveMasterBrowser:(NetBiosCompletionBlock)aCompletion{
    [SMB4iOSNetBios performOnTreadWithRunLoopInDefaultModeBlock: ^{
          ...
          completion:(NetBiosCompletionBlock)aCompletion
{
    ...
    SMB4iOSNetBiosOperation *op = [[SMB4iOSNetBiosOperation alloc] init];
    op.completion = aCompletion;
    op.tag = [self nextOperationTag];
    op.serverName = name;
    
    SMB4iOSNetBiosQuery *query = [[SMB4iOSNetBiosQuery alloc] init];
    query.nbtName = name;
               ...
               port:(UInt16)port
{
    ...
    SMB4iOSNetBiosOperation *op = [_operations objectForKey:@(tag)];
    if (result) {
...
- (void)onUdpSocketDidClose:(SMB4iOSAsyncUdpSocket *)sock{
    ...
    [operations enumerateKeysAndObjectsUsingBlock:^(NSNumber * _Nonnull key,
                                                    SMB4iOSNetBiosOperation * _Nonnull obj,
                                                    BOOL * _Nonnull stop)
     {
        SMB4iOSNetBiosOperation *op = obj;
        if (op.completion) {
            op.completion(nil);
        }
    }];
...
@implementation SMB4iOSNetBiosOperation
