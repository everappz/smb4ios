#import "SMB4iOSSmbConnection.h"
#import "SMB4iOSSmbMessage.h"
#import "SMB4iOSSmbNegotiateProtocolMessage.h"
#import "SMB4iOSSmbSessionSetupMessage.h"
#import "SMB4iOSSmbTreeConnectMessage.h"
#import "SMB4iOSSmbTreeDisconnectMessage.h"
#import "SMB4iOSSmbNtCreateMessage.h"
#import "SMB4iOSSmbTransactionMessage.h"
#import "SMB4iOSSmbReadMessage.h"
#import "SMB4iOSSmbWriteMessage.h"
#import "SMB4iOSSmbCloseMessage.h"
#import "SMB4iOSSmbLogoffMessage.h"

#import "SMB4iOSntlm.h"
#import "SMB4iOSDceRpc.h"
#import "SMB4iOSRapNetServerEnum2.h"
...
//#define LOGDATA(data) [SMB4iOSUtils logData:data]
#define LOGDATA(data)


@implementation SMB4iOSSmbConnection
{
	int pid;
	int mid;
	int uid;
...
{
	NSLog(@"Method not supported by SMB4iOSSmbConnection");
	assert(false);
}
...
- (bool) runSmb:(SMB4iOSSmbMessage *)smb
{
	LOG(@"%@", [smb class]);
...
- (bool) runRpc:(SMB4iOSDceRpcMessage *)rpc
{
	...
	while (frag < rpc.requestFragments-1)
	{
		...
		SMB4iOSSmbWriteMessage *write = [[SMB4iOSSmbWriteMessage alloc] init];
		write.fid = fid;
	...
	// Run Transact first
	SMB4iOSSmbTransactMessage *transaction = [[SMB4iOSSmbTransactMessage alloc] init];
	...
	SMB4iOSSmbReadMessage *read = [[SMB4iOSSmbReadMessage alloc] init];
	read.fid = fid;
...
{
	// Negotiate Protocol
	SMB4iOSSmbNegotiateProtocolMessage *negotiate = [[SMB4iOSSmbNegotiateProtocolMessage alloc] init];
	...
	if (anonymous)
	{
		SMB4iOSSmbSessionSetupMessage *sessionSetup = [[SMB4iOSSmbSessionSetupMessage alloc] init];
	...
	else
	{
        ...
        SMB4iOSSmbSessionSetupMessage *sessionSetupN = [[SMB4iOSSmbSessionSetupMessage alloc] init];
        ...
        SMB4iOSSmbSessionSetupMessage *sessionSetupA = [[SMB4iOSSmbSessionSetupMessage alloc] init];
	...
	// Tree Connect
	SMB4iOSSmbTreeConnectMessage *treeConnect = [[SMB4iOSSmbTreeConnectMessage alloc] init];
...
- (bool) treeDisconnect
{
	SMB4iOSSmbTreeDisconnectMessage *treeDisconnect = [[SMB4iOSSmbTreeDisconnectMessage alloc] init];
	if (![self runSmb:treeDisconnect])
		return false;

	SMB4iOSSmbLogoffMessage *logoff = [[SMB4iOSSmbLogoffMessage alloc] init];
...
{
	SMB4iOSSmbNtCreateMessage *ntCreate = [[SMB4iOSSmbNtCreateMessage alloc] init];
	...
	SMB4iOSDceRpcBind *rpcBind = [[SMB4iOSDceRpcBind alloc] init];
...
- (bool) enumServers:(UInt32)serverType domain:(NSString *)domain
{
	...
	SMB4iOSRapNetServerEnum2 *netServerEnum2 = [[SMB4iOSRapNetServerEnum2 alloc] init];
...
- (bool) enumShares
{
	...
	SMB4iOSDceRpcEnumAll *rpcEnumAll = [[SMB4iOSDceRpcEnumAll alloc] init];
	rpcEnumAll.host = self.host;
	if (![self runRpc:rpcEnumAll])
		return false;
		
	SMB4iOSSmbCloseMessage *close = [[SMB4iOSSmbCloseMessage alloc] init];
	close.fid = fid;
...
- (bool) getPrinter:(NSString *)printerName
{
	...
	SMB4iOSDceRpcOpenPrinterEx *rpcOpenPrinter = [[SMB4iOSDceRpcOpenPrinterEx alloc] init];
	...
	SMB4iOSDceRpcGetPrinter *rpcGetPrinter;

	do
	{
		rpcGetPrinter = [[SMB4iOSDceRpcGetPrinter alloc] init];
	...
	SMB4iOSDceRpcClosePrinter *rpcClose = [[SMB4iOSDceRpcClosePrinter alloc] init];
...
- (bool) startPrint:(NSString *)printerName
{
	...
	SMB4iOSDceRpcStartDocPrinter *rpcStartDoc = [[SMB4iOSDceRpcStartDocPrinter alloc] init];
...
- (bool) write:(NSData *)message // override
{
	...
	if (printHandle == NULL)
	{
		NSLog(@"SMB4iOSSmbConnection: no printer opened");
		return false;
	}

	SMB4iOSDceRpcWritePrinter *rpcWritePrinter = [[SMB4iOSDceRpcWritePrinter alloc] init];
...
- (bool) endPrint
{
	...
	SMB4iOSDceRpcEndDocPrinter *rpcEndDoc = [[SMB4iOSDceRpcEndDocPrinter alloc] init];
